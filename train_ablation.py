...
def main():
    ...
    # 存储每次实验的结果
    args.superpixel_nums = 100
    args.trimap_alpha = 10
    ...
    if args.num_trials > 1:
        ...
        if args.alphalabel:
            filename += f"_ta{args.trimap_alpha}"
        if args.matting == False:
            filename += "_no_matting"
        if args.random_superpixel:
            filename += "_rs"
        ...
        # 保存为CSV格式
        with open(f"results/ablation/{filename}.csv", 'w') as f:
